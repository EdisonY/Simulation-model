<template>
  ...
  >
    ...
    <!-- <el-tabs type="border-card" style="margin:10px 0 0 10px">
                ...
                <el-tab-pane label="输入开行方案">
                    ...
                    <p style="text-align:right;padding:20px 0">
                        <el-button type="primary">添加条数</el-button>
                        <el-button type="success">铺画</el-button>
                    </p>
                ...
                <el-tab-pane label="修改运行图">
                    ...
                    <p style="text-align:right;padding:20px 0">
                        <el-button type="danger">删除车次</el-button>
                        <el-button type="primary">新建车次</el-button>
                    </p>
                </el-tab-pane>
            </el-tabs> -->
    ...
    <!-- </div> -->
    <div class="all" :class="{ isMove: openLeft }">
      <div class="all1">
        <div
          class="calcH move1"
          ref="aaa"
    ...
    <div class="tooltip" v-if="rungrapData.multiply">
      ...
      <span class="leave1 legend" v-show="loadRateMode">&lt;40%</span>
      <span class="leave2 legend" v-show="loadRateMode">&lt;60%</span>
      <span class="leave3 legend" v-show="loadRateMode">&lt;80%</span>
      <span class="leave4 legend" v-show="loadRateMode">&lt;100%</span>
      <span class="leave5 legend" v-show="loadRateMode">&gt;=100%</span>

      <span class="leave1 legend" v-show="!loadRateMode">&lt;0s</span>
      <span class="leave2 legend" v-show="!loadRateMode">&lt;120s</span>
      <span class="leave3 legend" v-show="!loadRateMode">&lt;300s</span>
      <span class="leave4 legend" v-show="!loadRateMode">&lt;600s</span>
      <span class="leave5 legend" v-show="!loadRateMode">&gt;=600s</span>
      ...
      <el-button size="mini" type="primary" @click="drawAllData">刷新</el-button>
      <!-- <el-button size="mini"
                type='danger'
            ...
            if (typeof params[x].value == "number") {
              ...
              if (params[x].value != undefined) {
                tooltipInner +=
                  ...
                  params[x].seriesName +
                  "-" +
                  self.getSiteName(Number(params[x].value.substr(1))) +
                  "-" +
                  params[x].name +
                  "</br>" +
                  "</span>";
      ...
      function splitData(data, nummm) {
        ...
        for (let index = 0; index < data.pathListStr.length; index++) {
          if (!data.pathListStr[index].stationId) {
            ...
            data.pathListStr[index].stationId = parseInt(pathArr[0]);
            data.pathListStr[index].arriveTime = secondToDate(pathArr[1]);
            data.pathListStr[index].departTime = secondToDate(pathArr[2]);
            ...
            //新增区间启停时间
            data.pathListStr[index].arriveTimeInterval = secondToDate(pathArr[7]);
            data.pathListStr[index].departTimeInterval = secondToDate(pathArr[8]);

            // 判断区间是否停车，插入区间停车数据
            if ((pathArr[7] != "0" || pathArr[8] != "0") && mode && index > 0) {
              var preStationID = data.pathListStr[index - 1].stationId;
              let intervalTmp = {
          ...
          if (data.pathListStr.length > 1) {
            if (index == 0) {
              ...
              if (curStationId == parseInt(next.stationId)) {
                if (data.runDir == 85) {
                  curStationId = parseInt(next.stationId) - 0.3;
                } else {
                  curStationId = parseInt(next.stationId) + 0.3;
                }
              ...
              if (data.pathListStr[index].stationId == pre.stationId) {
                if (data.runDir == 85) {
                  data.pathListStr[index].stationId = pre.stationId + 0.3;
                } else {
                  data.pathListStr[index].stationId = pre.stationId - 0.3;
                }
              }
        ...
        for (let x = 0; x <= timeArr[timeArr.length - 1]; x++) {
          if (x != timeArr[timeArrIndex]) {
            ...
            if (timeArrIndex >= 1) {
              let multiply = data.pathListStr[pathListIndex].loadRate00 / 10000;
              let diviationTime = data.pathListStr[pathListIndex].diviationTime / 10000;
              let lateTime = data.pathListStr[pathListIndex].lateTime;
              ...
              let formatter = timeArrIndex == 2 ? "" + data.tripNo : "";
              ...
              if (self.rungrapData.multiply) {
                ...
                dataColl.markLine.data[timeArrIndex - 1][2].lineStyle.color = "#000";
                if (nummm != 0) {
            ...
            // 场段出车不停站时间相同
            if (timeArr[timeArrIndex] && x == timeArr[timeArrIndex]) {
              x--;
            }
          }
        }
        ...
        if (dataColl.markLine.data[dataColl.markLine.data.length - 1].length == 1) {
          //防止最后一个画线不能赋值，直接删除
