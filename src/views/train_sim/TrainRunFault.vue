  ...
  <div class="main fault">
      ...
      <el-card class="box-card" style="max-height: 1000px">
        ...
        >
          ...
          <div class="fault-info" v-else-if="faultID >= 200">
            <el-form-item
              v-for="faultInfo2 in faultInfoItem[1]"
          ...
          <div class="fault-info" v-else-if="faultID >= 100">
            <el-form-item
              v-for="faultInfo3 in faultInfoItem[0]"
          ...
          <el-form-item label="车次号" v-if="formLabelAlign.fault == 1">
            <el-input v-model="formLabelAlign.name"></el-input>
          </el-form-item>
          <el-form-item label="位置" v-if="formLabelAlign.fault == 1">
            <el-input v-model="formLabelAlign.station"></el-input>
          </el-form-item>
          <el-form-item label="位置" v-if="formLabelAlign.fault == 3">
            <el-input v-model="formLabelAlign.station2"></el-input>
          </el-form-item> -->
          <!-- <el-form-item label="选择车站" v-if="formLabelAlign.fault == 0">
          ...
          <!-- <el-form-item label="持续时间" v-if="formLabelAlign.fault != 0">
    ...
    <div class="main-right" style="padding-left: 10px">
      ...
      <div style="position: absolute; margin-top: 10px; margin-left: 10px">
        <el-button type="primary" size="mini" style="margin: 10px" @click="onRestLine"
          >重置线路</el-button
        >
      </div>
      <div class="msgTip" style="visibility: hidden">
        <div v-for="(data, index) in tooltipData">
          {{ data.name }}：{{ transTrainDataDisplay(data) }}
        </div>
      </div>
      <div class="info-panel">
        <div class="info" v-if="currentFrame">
          <div style="flex: 1">
        ...
        <div class="time-axis">
          <div class="slider">
            ...
            <span style="margin: 0 20px 0 10px; color: #2fd991; font-weight: bold"
          ...
          <div class="handle">
            <div class="player">
              ...
              <span style="margin: 0 20px 0 10px; color: #2fd991; font-weight: bold"
                >当前倍速:{{ speedRate }}</span
              >
              <el-radio v-model="speedRate" :label="1">x1</el-radio>
              <el-radio v-model="speedRate" :label="2">x2</el-radio>
              <el-radio v-model="speedRate" :label="4">x4</el-radio>
              <el-radio v-model="speedRate" :label="8">x8</el-radio>
              <el-radio v-model="speedRate" :label="16">x16</el-radio>
              <el-radio v-model="speedRate" :label="32">x32</el-radio>
              <el-radio v-model="speedRate" :label="64">x64</el-radio>
              <el-radio v-model="speedRate" :label="128">x128</el-radio>
            </div>
          </div>
        </div>
      </div>
      <div class="waring-panel" v-if="warningListArr && warningListArr.length > 0">
        <p v-for="msg in warningListArrDisplay" style="direction: ltr">{{ msg }}</p>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      faultInfo: [
        //故障信息数组
        {
          faultTrainID: 1005, //故障列车ID
          faultInterval: "阎村东(下行)-紫草坞(下行)", //故障区间
          faultLink: 44, //故障触发LINK”
          faultOffset: 0, //故障触发OFFSET”
          // refreshPlanFlag: 0, //是否前端分配故障车后续运行计划，0：否由后端自动处置 1：前端更新运行计划
          dealFlag: 1, //故障处置优先标志 1：优先回库 2：优先进入存车线 0:无效
        },
        {
          faultZCID: 1, //故障ZCID 1右侧ZC故障 2左侧ZC故障
          faultTime: 24000, //故障发生时间 从0点计算的s数
          recoverTime: 34000, //故障解除时间 从0点计算的s数
          releaseStationTime: 22000, //解除扣车时间，必须在故障发生时间和故障解除时间之间
        },
        {
          ...
          faultLink: 44, //故障触发LINK”
          faultOffset: 0, //故障触发OFFSET”
          recoverTime: 25000, //故障解除时间 从0点计算的s数
        },
      ],
      faultInfoItem: [
        //故障信息数组
        [
          ...
          {
            key: "dealFlag",
            label: "故障处置优先标志", //故障处置优先标志 1：优先回库 2：优先进入存车线 0:无效]
          },
        ],
        [
          {
            key: "faultZCID", //故障ZCID 1右侧ZC故障 2左侧ZC故障
            label: "故障ZCID",
          },
          {
            key: "faultTime", //故障发生时间 从0点计算的s数
            label: "故障发生时间",
          },
          {
            key: "recoverTime", //故障解除时间 从0点计算的s数
            label: "故障解除时间",
          },
          {
            key: "releaseStationTime", //解除扣车时间，必须在故障发生时间和故障解除时间之间
            label: "解除扣车时间",
          },
        ],
        [
          ...
          {
            key: "faultInterval", //故障区间
            label: "故障区间",
          },
          // {
          //   key: "RRMDelayTime", //应答器故障触发后列车停稳后转入RRM模式所需时间
          //   label: "进入故障延时",
          // },
          {
            key: "faultLink", //故障触发LINK”
            label: "故障触发LINK",
          },
          {
            key: "faultOffset", //故障触发OFFSET”
            label: "故障触发OFFSET",
          },
          {
            key: "recoverTime", //故障解除时间 从0点计算的s数
  ...
  methods: {
    ...
    faultInjection() {
      ...
      if (this.faultID >= 100 && this.faultID < 200) {
        faultIndex = 0;
      } else if (this.faultID >= 200 && this.faultID < 300) {
        faultIndex = 1;
      } else if (this.faultID >= 300 && this.faultID < 400) {
        faultIndex = 2;
      }
      if (faultIndex == -1) {
        this.$message({
...
<style scoped>
.fault .el-radio.is-bordered + .el-radio.is-bordered {
  margin-left: 0;
}
.fault .el-radio--mini.is-bordered {
  margin-bottom: 10px;
}
.fault .el-select {
  width: 248px;
