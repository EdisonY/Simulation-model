...
import ChartReport from "./controls/ChartReport";
...
export default {
    ...
    data() {
        return {
            ...
            indicator: {
                ...
                lineName: "",
                graphId: "\u0001",
                generateTime: "09:20:30",
                xStationNames: [],
                x1AblitySummary: [],
                y1AblitySummary: [[80, 120]],
                t1AblityDetail: {
                    x1AblitySummary: [],
                    y1AblitySummary: [],
                    stationPassTimeInterval100: [1, 2],
                    zonePassTimeInterval100: [],
                    ReverseTimerInterval100: [],
                },
                y3PassengerAverageWaitingTime: [[]],
                x4CapacityAndVolume: [],
                y4CapacityAndVolume: [],
                y5morningPeaks: [],
                y6maxSectionOd: [],
                x7EnergySummary: ["牵引能耗（万度）", "图定公里"],
                y7EnergySummary: [[5800, 2800]],
                x8EnergyTraction: [
                    "1001",
                    "1002",
                    "1003",
                ...
                y8EnergyTraction: [
                    [338, 229, 641, 470, 558, 470, 330, 544],
                    [603, 254, 246, 398, 650, 421, 591, 461],
                    [577, 629, 403, 406, 592, 395, 472, 386],
                ],
                x9Speed: [],
                y9Speed: [],
            },
    ...
    created() {
        registerCallback("ability", this.wsCallback);
    },
    beforeDestroy() {
        unregisterCallback("ability");
    },
    async mounted() {
    ...
    methods: {
            ...
            if (action === "add") {
                ...
                if (this.trainInfo.length > 0) {
                    ...
                    this.trainInfo.push({
                        ...
                        name: newTabName,
                        key: newTabName,
                        startArea: last.startArea,
                        reverseArea: last.reverseArea,
                        endArea: last.endArea,
                        uniformStopTime: last.uniformStopTime,
                        stopAreas: _.cloneDeep(last.stopAreas),
                        direction: last.direction,
                        currentTrain: last.currentTrain,
                    });
                    console.log(last);
                } else {
                    this.trainInfo.push({
        ...
        onRunAbility() {
            ...
            console.log("--- TODO test param ---");
            console.log(param);

            sessionStorage.setItem("summaryMode", 1);
            sessionStorage.setItem("cfgAbilityParam", JSON.stringify(param));

            let test = { trainList: [] };
            test.trainInfo = this.trainInfo;
            test.runMode = this.runMode;
            sessionStorage.setItem("cacheAbility1", JSON.stringify(test));

            let needSend = getPackage(131, param);
            sendSock(needSend);
        },
        _sbFunc(sid) {
            if (this.project) {
        ...
        wsCallback(res) {
            if (res.msgType == 206) {
                ...
                this.refreshChart++;
                if (this.$refs.chartReport) {
                    this.$refs.chartReport.initData();
                }
                ...
                if (res.status == 1) {
                    ...
                    this.trainLinkList = saveData.trainList;
                    this.showPath();
                    this.getChartData();
                } else {
                    this.$message({
                ...
                let cacheAbility1 = sessionStorage.getItem("cacheAbility1");
                if (cacheAbility1) {
                    cacheAbility1 = JSON.parse(cacheAbility1);
                    this.trainLinkList = cacheAbility1.trainList;
                    this.trainInfo = cacheAbility1.trainInfo;
                    this.runMode = cacheAbility1.runMode;
                    this.showPath();
                    this.getChartData();
                }
            }
