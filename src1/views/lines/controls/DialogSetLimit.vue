    ...
    <el-dialog title="设置限速"
        ...
        <div class="main">
            <el-table ref="table"
                ...
                <el-table-column label="缓和曲线长度(m)"
                    width="120">
                    <template slot-scope="scope">
                        <el-input size="mini"
                            v-model="scope.row.L"></el-input>
                    </template>
                </el-table-column>
            ...
            <div class="footer">
                ...
                <div class="btn-handle">
                    <span style="color:rgb(64,158,255)">统一限速(km/h)</span>
                    <el-input size="mini"
                        style="margin-left: 20px; width: 80px"
                        v-model="current.uniformLimitValue"></el-input>

                    <span style="color:rgb(64,158,255);margin-left:50px">车站限速(km/h)</span>
                    <el-input size="mini"
                        style="margin-left: 20px; width: 80px"
                        v-model="current.platFormV"></el-input>
                </div>
                <div class="btn-handle">
                    <el-button size="mini"
...
export default {
    props: ["limitInfo", "dialogFormVisible", "maxV"],
    data() {
        return {
            modalClose: false,
            current: _.cloneDeep(this.limitInfo),
            directions: [
                {
                    name: "上行",
                    value: 1,
    ...
    methods: {
        dialogAction(action) {
            if (action == "cancelAction") {
                this.current = _.cloneDeep(this.limitInfo);
            } else {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        for (let i = 1; i < rows.length - 1; i++) {
                            grades.push({
                                id: rows[i][0],
                                startCm: Number(rows[i][1]),
                                endCm: Number(rows[i][2]),
                                R: Number(rows[i][3]),
                                L: Number(rows[i][4]),
                                V: Number(rows[i][5]),
                                H: Number(rows[i][6]),
                                direction: Number(rows[i][7]),
                            });
                        }
